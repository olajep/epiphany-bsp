...
void* ebsp_stream_create(int processor_id, int stream_size, int token_size,
    ...
    // 3) add stream to state
    _ebsp_add_stream(processor_id, extmem_buffer, nbytes_including_headers,
                     token_size, 0);
...
void _ebsp_add_stream(int core_id, void* extmem_buffer, int nbytes,
                      int max_chunksize, int is_down_stream) {
